...
export const TestsHistory = ({ data }: TestHistories) => {
  ...
  return (
    <section aria-label="Load Tests" role="table" className="w-full flex flex-col">
      {Object.entries(groupedTests).length === 0 && <TestNotFound />}
      {Object.entries(groupedTests).map(([createdDate, tests]) => (
          ...
          {tests.map(test => (
            ...
            >
              <div className="flex items-center gap-x-2 lg:gap-x-3">
                <TestStatus status={test.status} currentReqs={2} totalReqs={2} />
                <div className="flex flex-col max-w-xs">
                  <div className="flex items-center gap-2">
                    <h3 className="text-[0.8125rem] text-neutral-600 dark:text-gray-300 truncate">
                      {test.user.name}
                    </h3>
                    <TestHistoryBadge notes={test.notes} />
                  </div>
