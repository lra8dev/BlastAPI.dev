...
type HeroCardProps = { isHistoryAvailable?: boolean };

export const HeroCard = ({ isHistoryAvailable }: HeroCardProps) => {
  return (
    <Card
      className={cn(
        "font-medium relative mx-auto my-2 flex w-11/12 max-w-3xl flex-col gap-3 rounded-md",
        "bg-white dark:bg-dark-2 p-8 min-[42rem]:p-12 md:p-20",
        "min-h-[280px] overflow-hidden shadow-none",
        isHistoryAvailable && "mt-26",
      )}
    >
      {/* Background Grid */}
      <div
